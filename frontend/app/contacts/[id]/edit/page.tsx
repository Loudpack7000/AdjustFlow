...
export default function EditContactPage() {
  ...
  // Load contact data and field definitions
  useEffect(() => {
    const loadData = async () => {
      try {
        setLoadingContact(true);
        // Load contact data
        const contactResponse = await contactsApi.get(contactId);
        ...
        setMainPhone(contact.main_phone || '');
        setMobilePhone(contact.mobile_phone || '');
        setAddressLine1(contact.address_line_1 || '');
        setAddressLine2(contact.address_line_2 || '');
        setCity(contact.city || '');
        setState(contact.state || '');
        setPostalCode(contact.postal_code || '');
        setContactType(contact.contact_type || '');
        setStatus(contact.status || '');
        setSalesRepId(contact.sales_rep_id);
        setLeadSource(contact.lead_source || '');
        setAssignedToIds(contact.assigned_to_ids || []);
        setDescription(contact.description || '');
        setNotes(contact.notes || '');
        setTags(contact.tags || []);
        setCustomerType(contact.customer_type || '');
        setTextingOptOut(contact.texting_opt_out || false);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setError(err?.response?.data?.detail || 'Failed to update contact. Please try again.');
    } finally {
      setLoading(false);
    }
  };
